...
#     context::MyBanditPortfolioAllocationContextModel = nothing)
...
#         end
#         r, _ = world(aₜ, context); # get the reward from the world
#         μ[i] = r;
...
#         rₜ, _ = world(aₜ, context); # get the reward from the world
...
# function _solve(model::MyEpsilonGreedyStaticNoiseAlgorithmModel; T::Int = 0, world::Function = _null, 
...
#             aₜ = digits(i - 1, base=2, pad=K); # generate a binary representation of the number, with K digits  
#         end
#         r,n,p,γ = world(1, aₜ, context); # get the reward from the world (use the first day of the OOS data)
#         μ[i] = r; # update the average reward for the chosen arm (learning rate = α)
...
#         # call out to the world, record the result.
#         rₜ, nₜ, pₜ, γₜ = world(1, aₜ, context); # get the reward from the world (use the first day of the OOS data)
#         R[t, î] = rₜ; # store the reward in the rewards array
